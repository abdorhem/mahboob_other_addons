...
class MaterialRequest(models.Model):
    ...
    allow_supevisor_ids = fields.Many2many('res.users', string='allow_supevisor')
    current_user = fields.Many2one('res.users',compute='_current_user', string='')
    is_allow = fields.Boolean(compute='_is_allow')
    ...
    def _is_allow(self):
        for rec in self :
            rec.is_allow = False
            objects = self.env['res.users'].sudo().search([('branch_ids','in',rec.branch_outlet_id.id)])
            if self.env.user.id in objects.ids :
               rec.is_allow = True 
    ...
    def create(self, vals):
        ...
        objects = self.env['material.request'].sudo().search([('source_outlet_id','=',res.source_outlet_id.id),
        ('branch_outlet_id','=',res.branch_outlet_id.id),
        ('transfer_count','>=',2),
        ('id','!=',res.id),
        ],limit=1)
        if objects :
            pickings = self.env['stock.picking'].search([('request_id', '=', objects.id)])
            for p in pickings :
                if p.state not in ('done','cancel') :
                    raise UserError(_('You have a non complete request with Number %s !') %(objects.name))
